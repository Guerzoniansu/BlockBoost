...
def _save_comprehensive_metrics(results_bb: dict, garch_metrics: dict,
    ...
    for alpha in alpha_levels:
        ...
        for a_T in block_sizes:
            ...
            row = {'dataset': label, 'model': 'Q-BB', 'block': a_T, 'alpha': alpha}
            for p in periods:
                m = m_periods[p]
...
def run_dataset(key: str):
    ...
    for a_T in BLOCK_SIZES:
        t0 = time.time()
        print(f'  [Q-BB   a_T={a_T:2d}] fitting …', end=' ', flush=True)
    ...
    for col in GARCH_COLS:
        ...
        print(f'  [{col}  alpha=0.01]  viol={m["global"]["viol_rate"]*100:.2f}%  '
    ...
    for col in GARCH_COLS:
        ...
        print(f'  [{col}  alpha=0.05]  viol={m["global"]["viol_rate"]*100:.2f}%  '
              f'Kup p={m["global"]["kupiec_p"]:.3f}')
    ...
    print(f'\n  [best block at alpha=5%] a_T={best_block}  '
